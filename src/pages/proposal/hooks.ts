...
import { Logger, normalizeTonAddress } from "utils";
import { useEnpointsStore } from "./store";
import { getClientV4 } from "ton-vote-contracts-sdk";
...
export const useWalletVote = (votes?: Vote[], dataUpdatedAt?: number) => {
  ...
  return useMemo(() => {
    const connected = normalizeTonAddress(walletAddress);
    return _.find(votes, (it) => normalizeTonAddress(it.address) === connected);
